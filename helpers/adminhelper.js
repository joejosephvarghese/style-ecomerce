...
module.exports={
  ...
  Blockuser:(Blockid)=>{
    return new Promise (async(resolve,reject)=>{
        const user = await users.updateOne({_id: Blockid},{$set:{status: false}})
        if(user){
            resolve(user)
        }else{
            resolve()
     ...
     Unblockuser:(Unblockid)=>{
      return new Promise(async(resolve,reject)=>{
        const user = await users.updateOne({_id:Unblockid},{$set:{status:true}})
         if (user){
          resolve(user)
         }else{
          resolve()
...
//   return new Promise((resolve,reject)=>{
//       Banner.findOne({_id : bannerId}).then((bannerFound)=>{
//       resolve(bannerFound)
//       })
//   })
// },
