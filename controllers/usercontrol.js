...
module.exports = {
  userHome: async(req, res) => {
    ...
    res.render("user/user-homepage", { user, data, categories, pages2,count,banner });
  },
  

  getLoginPage: (req, res) => {

    res.render("user/user-loginpage" ,{ layout: "layout", login: true,"loggnerr":req.session.logginerr });
  ...
  getshop_product:async(req,res)=>{
   ...
   try{
    res.status (200).render('../views/user/shop_product',{user,product,count});
   }catch (error){
       ...
       if(quantity?.totalQuantity){
        var Q=quantity.totalQuantity
         ...
         var Q =0;
            ...
            let S= stock.stock
            console.log(S,"jjjjjjjjjjjj");
            const result= S-Q


            console.log(result,"yesssssssssss");
            if(result>0){
  ...
  let total = await userhelper.totalCheckOutAmount(userId)
     ...
     res.render('../views/user/cart-page',{user,cartdetails,total,count});
   ...
   updateQuantity:(req,res)=>{
    ...
    userhelper.updateQuantity(req.body).then(async (response) => {
      ...
      // response.subTotal = await orderHelpers.getSubTotal(userId)
          res.json({newQuantity:response.newQuantity, status: true, total: total})
      })
   },
...
getcatagorypage:async(req,res)=>{
  ...
  userhelper.getcatafilter(req.params).then((result)=>{
    ...
    res.render('../views/user/filter',{user,result,categories,count})
...
Edit_Address: async (req, res) => {
 ...
 console.log(user._id,editdata,"my");
...
Edited_Address: async (req, res) => {
  try {
    ...
    const addressId = req.params.id
     ...
     await orderhelper.post_EditedAddress(data, user,addressId).then((data)=>{
      console.log(data,"K");
...
applyCoupon: async(req,res)=>{
    ...
    getWishList: async (req,res)=>{
      ...
      const userId=user._id
      let count = await userhelper.getCartItems(userId)
      ...
      wishListHelpers.getWishListProducts(userId).then((wishlistProducts)=>{
        console.log(wishlistProducts,"love");
          res.render('../views/user/whishlist',{user,count,wishlistProducts,wishlistCount})
      })
  },
